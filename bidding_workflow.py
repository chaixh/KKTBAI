...
class BiddingWorkflow:
    def __init__(self):
        ...
        # 核心新增：存储完整终稿内容，供app.py获取
        self.full_document_content = ""
        # 核心新增：终稿保存路径，方便追踪
        self.document_save_path = Config.OUTPUT_DIR / 'content.md'
    ...
    async def generate_full_content_async(self) -> bool:
        """异步生成完整文档内容（核心修改：存储真实内容到self.full_document_content）"""
...
def generate_document():
    ...
    try:
        ...
        return jsonify({
            ...
            "document_content": workflow.full_document_content,  # 返回真实内容
